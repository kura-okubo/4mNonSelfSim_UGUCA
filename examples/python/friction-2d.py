...
interface = ug.UnimatShearInterface(mesh, {ug.x, ug.y},top_mat, law)
...
mus = law.getMuS()
mus[:] = mu_s + mus_ampl*(1.0*np.sin(2*coords[0, :]*np.pi) +
                          0.5*np.cos(12*coords[0, :]*np.pi) +
                          0.7*np.sin(18*coords[0, :]*np.pi))
...
loads[0, :] = mus_min*np.abs(normal_load)
loads[1, :] = normal_load

interface.init(False)

interface.initDump('friction_2d_example', '.')
interface.registerDumpFields('cohesion,top_disp,top_velo,mu_s')
interface.dump(0, 0)
...
for s in range(nb_time_steps):
    loads[0, :] = mus_min*np.abs(normal_load) + s*time_step*shear_load_rate 
