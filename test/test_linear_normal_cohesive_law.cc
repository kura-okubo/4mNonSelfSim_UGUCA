...
int main(){
  ...
  NodalFieldComponent & Sigmac_tmp = law.getSigmac();
  if (std::abs(Sigmac_tmp.at(0) - sigmac) / sigmac > 1e-5){
    std::cout << "wrong sigmac: " << Sigmac_tmp.at(0) << std::endl;
    return 1; // failure
  }
  ...
  NodalFieldComponent & sigma0 = load.component(1);
  HalfSpace & top = interface.getTop();
  NodalFieldComponent & u1 = top.getDisp().component(1);
