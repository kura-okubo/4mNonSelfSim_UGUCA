/**
 * @file   test_unimat_normal_interface.cc
 *
...
int checkClosingNormalGapForce(SimpleMesh &mesh, UnimatNormalInterface &interface,
                               NodalField &fields) {
  ...
  std::vector<double> internal_1;
  internal_1.reserve((size_t)mesh.getNbLocalNodes());
  ...
  for (size_t i = 0; i < (size_t)mesh.getNbLocalNodes(); ++i) {
    ...
    random_value = unif(re);
    internal_1.push_back(random_value);
    top.getInternal().component(1).set(i) = random_value;
  }
  // compute reference closing forces, note that internal is not zero.
  ...
  for (size_t i = 0; i < (size_t)mesh.getNbLocalNodes(); ++i) {
    double du_1_top =  load_1[i] + internal_1[i];// is zero
    ref_close_force.push_back( u_1[i] / fact_top + du_1_top);
  }
  // check results
  double tol = 1e-10;
...
int checkMaintainShearGapForce(SimpleMesh &mesh, UnimatNormalInterface &interface,
                               NodalField &fields) {
  ...
  fact_top /= fact_both;
  for (size_t i = 0; i < (size_t)mesh.getNbLocalNodes(); ++i) {
    ref_maintain_force_0.push_back(load_0[i] );
    ref_maintain_force_2.push_back(load_2[i] );
  }
  // check results
...
int checkComputeGaps(SimpleMesh &mesh, UnimatNormalInterface &interface,
                     NodalField &fields) {
  // assign random u
  HalfSpace &top = interface.getTop();
  std::uniform_real_distribution<double> unif(0, 1);
  ...
  for (size_t i = 0; i < (size_t)mesh.getNbLocalNodes(); ++i) {
    if ((std::abs(fields.component(0).at(i)-disp[0][i] * 0) > tol) ||
	(std::abs(fields.component(1).at(i)-disp[1][i] * 2) > tol) ||
	(std::abs(fields.component(2).at(i)-disp[2][i] * 0) > tol)) {
  ...
  for (size_t i = 0; i < (size_t)mesh.getNbLocalNodes(); ++i) {
    if ((std::abs(fields.component(0).at(i)-velo[0][i] * 0) > tol) ||
	(std::abs(fields.component(1).at(i)-velo[1][i] * 2) > tol) ||
	(std::abs(fields.component(2).at(i)-velo[2][i] * 0) > tol)) {
...
int main(){
  ...
  UnimatNormalInterface interface(mesh, material_top, law);
  ...
  // --------------------------------------------------------------
  std::cout << "check UnimatNormalInterface::closingNormalGapForce" << std::endl;
  if (checkClosingNormalGapForce(mesh, interface,fields)) {
    std::cout << "UnimatNormalInterface::closingNormalGapForce failed"
              << std::endl;
    return 1;  // failed
  } else {
    std::cout
        << "UnimatNormalInterface::closingNormalGapForce correct -> success"
        << std::endl;
  }
  // --------------------------------------------------------------
  ...
  // --------------------------------------------------------------
  std::cout << "check UnimatNormalInterface::maintainShearGapForce" << std::endl;
  if (checkMaintainShearGapForce(mesh, interface, fields)) {
    std::cout << "UnimatNormalInterface::maintainShearGapForce failed"
              << std::endl;
    return 1;  // failed
  } else {
    std::cout
        << "UnimatNormalInterface::maintainShearGapForce correct -> success"
        << std::endl;
  }
  // --------------------------------------------------------------
