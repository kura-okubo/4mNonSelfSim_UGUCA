/**
 * @file   test_linear_coulomb_friction_law_selfhealing.cc
 *
...
int main(){
  ...
  std::cout << "start test: test_liner_coulomb_friction_law_coshselfhealing" << std::endl;
  ...
  LinearCoulombFrictionLawCoshSelfHealing law(mesh, mus, muk, dc, ds, T);
  ...
  NodalFieldComponent & Ds_tmp = law.getDs();
  if (std::abs(Ds_tmp.at(0) - ds) / ds > 1e-5){
    std::cout << "wrong ds: " << Ds_tmp.at(0) << std::endl;
    return 1; // failure
  }
  ...
  printf("debug test coh0=%e\n", coh0.at(0));

  // // check strengthening: tau0 > tauc & dc < ds < u
  tau0v = 100*mus*sig; // large enough to activate the cohesion
  tau0.setAllValuesTo(tau0v);
  u0v = 1.05*ds;
  u0.setAllValuesTo(u0v);
  double k_heal = 8e6; // emperically selected
  printf("debug test k_heal=%e\n", k_heal);
  val = ( (std::cosh(k_heal * (u0v - ds)) - 1) + muk ) * sig;
  law.computeCohesiveForces(cohesion, false);
  printf("err=%e\n", abs(coh0.at(0) - val) / val);
  std::cout << val << coh0.at(0) << std::endl;
  if ((std::abs((coh0.at(0) - val) / val) > 1e-5) || (coh0.at(0) * tau0v < 0)) {
    std::cout << "residual failed (" << val << "): " << coh0.at(0) << std::endl;
    return 1; // failure
  }
