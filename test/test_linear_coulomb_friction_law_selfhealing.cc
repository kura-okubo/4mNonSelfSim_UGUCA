...
int main(){
  ...
  std::cout << "start test: test_liner_coulomb_friction_law_selfhealing" << std::endl;
  ...
  LinearCoulombFrictionLawSelfHealing law(mesh, mus, muk, dc, ds, T);
  ...
  u0v = 1.2*ds;
  u0.setAllValuesTo(u0v);
  double kuv = 10*(mus-muk)/dc;
  printf("debug test kuv=%e\n", kuv);
  val = ( kuv * (u0v - ds) + muk )*sig;
  law.computeCohesiveForces(cohesion, false);
