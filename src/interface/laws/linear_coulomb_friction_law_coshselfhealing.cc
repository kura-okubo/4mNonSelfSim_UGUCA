/**
 * @file   linear_coulomb_friction_law_coshselfhealing.cc
 *
 ...
 * 
 * Updated 2024.5- Kurama Okubo for self-healing slip strengthening
 * Updated 2025.11.27 - Kurama Okubo for smooth self-healing slip strengthening using cosh function
 */

#include "linear_coulomb_friction_law_coshselfhealing.hh"
...
/* -------------------------------------------------------------------------- */
LinearCoulombFrictionLawCoshSelfHealing::LinearCoulombFrictionLawCoshSelfHealing(BaseMesh & mesh,
						   double mu_s_default,
						   double mu_k_default,
						   double d_c_default,
						   double d_s_default,
  ...
  reg_cont_pres_tmp(mesh)
{
  ...
  this->d_s.setAllValuesTo(d_s_default);
  this->char_time.setAllValuesTo(char_reg_time);
}

/* -------------------------------------------------------------------------- */
void LinearCoulombFrictionLawCoshSelfHealing::computeCohesiveForces(NodalField & cohesion,
						     bool predicting) {
  ...
  for (int n = 0; n<this->mesh.getNbLocalNodes(); ++n) {
    ...
    // UPDATE: case statement to set the slip-strengthening after ds
    double mu;
    if (shear_gap[n] < ds[n]) {
      double dmu = (1 - shear_gap[n] / dc[n]) * (mus[n] - muk[n]);
      mu = std::max(muk[n], muk[n] + dmu);
    } else {
      // compute slope of slip-strengthening
      // double ku = 10 * (mus[n]-muk[n])/dc[n]; //2024.06.15 100 -> 10
      // printf("debug ku=%e\n", ku);
      // printf("shear_gap=%e, ds=%e, muk=%e, reg_sig=%e\n", shear_gap[n], ds[n], muk[n], reg_sig[n]);
      // mu = ku * (shear_gap[n] - ds[n]) + muk[n];
      double k_heal = 8e6; // emperically selected
      mu = (std::cosh(k_heal * (shear_gap[n] - ds[n])) - 1) + muk[n];
    }
...
/* -------------------------------------------------------------------------- */
void LinearCoulombFrictionLawCoshSelfHealing::computeRegContactPressure(NodalFieldComponent & cohesion_1,
...
/* -------------------------------------------------------------------------- */
void LinearCoulombFrictionLawCoshSelfHealing::registerDumpField(const std::string & field_name) {
