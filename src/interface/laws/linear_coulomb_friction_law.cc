...
/* -------------------------------------------------------------------------- */
void LinearCoulombFrictionLaw::computeCohesiveForces(bool predicting, unsigned int ts_factor)
{
  ...
  for (int n = 0; n < this->mesh.getNbLocalNodes(); ++n)
  {
    ...
    if (this->char_time(n) > 0)
    {
      ...
      if (std::abs(cohesion(n, 1)) < 1e-12)
        ...
        reg_cont_pres(n) = (reg_cont_pres(n) + dt / this->char_time(n) * cohesion(n, 1)) / (1 + dt / this->char_time(n));
    }
    // not regularized
    else
    {
