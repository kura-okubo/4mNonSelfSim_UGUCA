...
def getData():
    ...
    with requests.Session() as session:
        ...
        res = session.post(url, data={'o': '1005', 'G1045tpv102': ' Select '})
        # print(res.content)
        soup = BeautifulSoup(res.content, "html.parser")
        groups = soup.find_all('input', {'type': 'submit', 'value': ' Select '})
        for group in groups:
            author = group.get('name')[5::]
            print('* ' + author)
            res = session.post(url, data={'m': 'tpv102', 'o': '1005', group.get('name'): ' Select '})
            soup = BeautifulSoup(res.content, "html.parser")
            files = soup.find_all('input', {'type': 'submit', 'value': 'Raw Data'})
            mus = soup.find('input', {'type': 'hidden', 'name': 'mus'}).get('value')
            for file in files:
                file_station = file.get('name')[5::]
                if file_station in stations:
                    print('\t' + file_station)
                    res = session.post(url, data={'m': 'tpv102', 'o': '1005', 'mus': mus, file.get('name'): 'Raw Data'})
                    soup = BeautifulSoup(res.content, "html.parser")
                    data = soup.find('pre')
