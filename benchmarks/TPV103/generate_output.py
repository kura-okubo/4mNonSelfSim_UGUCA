...
def dump_station(full_path, bname, station):
  ...
  with open('%s/ke_%s.txt' % (dump_path, station), 'w') as f:
    meta = [
      ...
      '# Column #9 = state variable (dimensionaless)']
    f.writelines('\n'.join(meta))
    f.write('# \n')
    f.write('t  h-slip  h-slip-rate  h-shear-stress  v-slip  v-slip-rate  v-shear-stress  n-stress  psi\n')
    
    for i in range(nt):
      f.write(('%20.12e' + '%14.6e' * 8 + '\n') % (t[i], delta[i], delta_dot[i], cohesion[i] * 1e-6, 0, 0, 0, -120, theta[i]))
